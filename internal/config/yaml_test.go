...
func TestYAMLLoader_Load_FilePickerMaxRecents(t *testing.T) {
	yamlContent := `
harnesses:
  - name: test
    command_template: "test"
...
`
	...
	if config.FilePickerMaxRecents != 10 {
		t.Errorf("Expected FilePickerMaxRecents to be 10, got %d", config.FilePickerMaxRecents)
	}
}

func TestYAMLLoader_Load_FilePickerMaxRecents_Default(t *testing.T) {
	yamlContent := `
harnesses:
  - name: test
    command_template: "test"
`
	...
	if config.FilePickerMaxRecents != 5 {
		t.Errorf("Expected FilePickerMaxRecents default to be 5, got %d", config.FilePickerMaxRecents)
	}
}

func TestYAMLLoader_Save_FilePickerMaxRecents(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "test.yaml")
	...
	config := &domain.Config{
		Harnesses: []domain.Harness{
			{Name: "test", CommandTemplate: "test"},
		},
		FilePickerMaxRecents: 15,
	}

	if err := loader.Save(configPath, config); err != nil {
		t.Fatalf("Failed to save config: %v", err)
	}
