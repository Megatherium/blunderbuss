...
// RecentsChangedMsg is emitted when the recents list changes.
type RecentsChangedMsg struct {
	Recents []string
}
		...
		case key.Matches(msg, m.KeyMap.Open):
			...
			if m.recentFocus {
				...
				if err == nil && stat.IsDir() {
					m.CurrentDirectory = m.Recents[m.recentSelect]
					m.pushView(m.selected, m.min, m.max)
					m.selected = 0
					m.min = 0
					m.max = m.Height - 1
					if cmd != nil {
						return m, tea.Batch(cmd, m.readDir(m.CurrentDirectory, m.ShowHidden))
					}
					return m, m.readDir(m.CurrentDirectory, m.ShowHidden)
				}
				return m, cmd
...
func (m Model) View() string {
	var leftPane string
	if len(m.files) == 0 {
		leftPane = m.Styles.EmptyDirectory.Height(m.Height).MaxHeight(m.Height).String()
	} else {
		...
		for i, f := range m.files {
			...
			if m.selected == i { //nolint:nestif
				...
				if m.recentFocus {
					cursorStyle = m.Styles.DisabledCursor
				}

				if disabled {
					s.WriteString(m.Styles.DisabledCursor.Render(m.Cursor) + m.Styles.DisabledSelected.Render(selected))
				} else {
					s.WriteString(cursorStyle.Render(m.Cursor) + m.Styles.Selected.Render(selected))
				}
				s.WriteRune('\n')
				continue
			}
	...
	r.WriteString(m.Styles.RecentsTitle.Render("Recent Files"))
	r.WriteRune('\n')

	if len(m.Recents) == 0 {
		r.WriteString(m.Styles.EmptyDirectory.Render("No recent files."))
		r.WriteRune('\n')
	} else {
		for i, path := range m.Recents {
			...
			if i == m.recentSelect {
				...
				r.WriteString(cursorStyle.Render(m.Cursor) + " " + m.Styles.Selected.Render(name))
			} else {
				r.WriteString(m.Styles.DisabledCursor.Render(" ") + " " + m.Styles.File.Render(name))
			}
			r.WriteRune('\n')
		}
	}
	...
	// pad to height
	for i := lipgloss.Height(r.String()); i <= m.Height; i++ {
		r.WriteRune('\n')
	}

	rightPane := lipgloss.NewStyle().PaddingLeft(4).Render(r.String())
